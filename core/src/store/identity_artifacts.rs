// Claude Code IDENTITY artifacts (skills / agents / slash-commands) stored per
// AI identity. The plugin pulls an identity's ENABLED artifacts via the sync
// endpoint, authenticated by the AI-identity token — keyed on the AI actor
// (people.slug), NOT the per-user memory namespace.
//
// The bare word `artifacts` means stored FILES (store/artifacts.rs). Table,
// type, store methods, tools, and routes here all carry the `identity_` prefix.
//
...
const IDENTITY_ARTIFACT_COLS: &str =
...
impl Store {
    ...
    pub async fn identity_artifacts_upsert(
        &self,
        actor: &str,
        kind: &str,
        name: &str,
        ...
        let row = crate::pgq::query(&format!(
            "INSERT INTO identity_artifacts (id, actor, kind, name, content, description, enabled, created_at, updated_at) \
             ...
             RETURNING {IDENTITY_ARTIFACT_COLS}"
        ))
        .bind(&id)
        .bind(actor)
        .bind(kind)
        ...
        .fetch_one(self.db())
        .await?;
        let artifact = row_to_identity_artifact(&row)?;
        self.emit(
            "identity_artifact.upserted",
            actor,
    ...
    pub async fn identity_artifacts_for_actor(&self, actor: &str) -> Result<Vec<IdentityArtifact>> {
        let rows = crate::pgq::query(&format!(
            "SELECT {IDENTITY_ARTIFACT_COLS} FROM identity_artifacts \
             WHERE actor = ? AND enabled = TRUE ORDER BY kind, name"
        ))
        .bind(actor)
        .fetch_all(self.db())
        .await?;
        rows.iter().map(row_to_identity_artifact).collect()
    }

    /// All artifacts for an actor (including disabled) — the management listing.
    pub async fn identity_artifacts_list(&self, actor: &str) -> Result<Vec<IdentityArtifact>> {
        let rows = crate::pgq::query(&format!(
            "SELECT {IDENTITY_ARTIFACT_COLS} FROM identity_artifacts \
    ...
    pub async fn identity_artifacts_get(&self, id: &str) -> Result<Option<IdentityArtifact>> {
        let row = crate::pgq::query(&format!(
            "SELECT {IDENTITY_ARTIFACT_COLS} FROM identity_artifacts WHERE id = ?"
        ))
        .bind(id)
        .fetch_optional(self.db())
        .await?;
        row.as_ref().map(row_to_identity_artifact).transpose()
    }

    pub async fn identity_artifacts_remove(&self, id: &str) -> Result<bool> {
...
fn row_to_identity_artifact(r: &sqlx::postgres::PgRow) -> Result<IdentityArtifact> {
    Ok(IdentityArtifact {
        id: r.try_get("id")?,

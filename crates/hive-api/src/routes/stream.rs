//! `/events/stream` ... Server-Sent Events stream of hive activity.
//!
...
//!
//! Moved from `/events` to `/events/stream` in migration 0012 so the `/events`
//! root is free for the new date-anchored events CRUD endpoints. The SSE shape

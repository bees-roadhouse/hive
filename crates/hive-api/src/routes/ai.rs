...
pub fn router() -> Router<AppState> {
    Router::new()
        .route("/ai", get(list_identities).post(create_identity))
        .route("/ai/{handle}/grants", post(create_grant))
        .route(
            "/ai/{handle}/grants/{user_id}",
            axum::routing::delete(delete_grant),
        )
        .route("/ai/{handle}/connect", post(connect))
        .route("/ai/{handle}/connections", get(ai_connections))
        .route("/me/ai-connections", get(my_connections))
        .route("/admin/ai-connections", get(admin_connections))
        .route("/ai/connections/{jti}/revoke", post(revoke_one))
...
/// `POST /ai/{handle}/connect`: a human owner connects AS an AI over MCP. Mints
...
/// `GET /ai/{handle}/connections`: all connections for an AI the caller owns.
...
/// `POST /ai/connections/{jti}/revoke`: revoke one token (scope #1, §5.5).

...
async fn who_redirect(axum::extract::Path(slug): axum::extract::Path<String>) -> Response {
    // Query the people endpoint by slug. Slugs are unique, so a hit yields
    // exactly one row; the kind discriminator tells us which canonical URL
    // to redirect to. We hit the API directly (rather than the client helper
    // in `api.rs`) because this handler runs outside the leptos render
    // context, so the SessionId-from-context plumbing doesn't apply.
    let url = format!("{}/people/{slug}", api::api_base());
    ...
    if !resp.status().is_success() {
        return who_not_found(&slug).into_response();
    }
    #[derive(Deserialize)]
    struct Row {
        slug: String,
    ...
    let row: Row = match resp.json().await {
        Ok(r) => r,
        Err(_) => return who_not_found(&slug).into_response(),
    };
    let target = match row.kind.as_str() {
        "ai" => format!("/ai/{}", row.slug),
        _ => format!("/people/{}", row.slug),
    };
    // Permanent: the kind is durable. If split-ai migrates a slug across
    // tables we'd want to update mention rendering too; the disambiguator
    // is the right place to absorb that change.

...
async fn metadata(State(s): State<Store>, Path(id): Path<String>) -> ApiResult {
    let org = s.orgs_default().await?;
...
async fn remove(State(s): State<Store>, Path(id): Path<String>) -> ApiResult {
    let org = s.orgs_default().await?;
    let Some(id) = parse_id(&id) else {
        return Ok(not_found());
    };
...
async fn content(State(s): State<Store>, Path(id): Path<String>, headers: HeaderMap) -> ApiResult {
    let org = s.orgs_default().await?;
    let Some(id) = parse_id(&id) else {
        return Ok(not_found());
    };

...
//
// The management routes (list-incl-disabled, upsert, delete) are gated to the
// actor's owner OR an admin: an AI's owner is the human in `people.owner`, so a
// caller may manage `{actor}` only when `ctx.is_admin()` or they ARE that
// actor's owner (`people.owner == ctx.actor()`).
...
/// True when the caller may manage `actor`'s artifacts: an admin, the actor
/// itself, or the actor's human owner (`people.owner`).
async fn can_manage(s: &Store, ctx: &AuthCtx, actor: &str) -> Result<bool, crate::error::ApiError> {
    if ctx.is_admin() {
        return Ok(true);
    }
    let me = ctx.actor();
    ...
    match s.people_get(actor).await? {
        Some(p) => Ok(p.owner.as_deref() == Some(me)),
        None => Ok(false),
    }
}
...
async fn list(
    ...
    if !can_manage(&s, &ctx, &actor).await? {
        return Ok(forbidden());
    }
...
async fn remove(
    ...
    if !can_manage(&s, &ctx, &artifact.actor).await? {
        return Ok(forbidden());
    }

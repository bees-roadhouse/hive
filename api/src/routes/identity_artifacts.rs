// Claude Code IDENTITY artifacts (skills / agents / slash-commands) per AI
// identity. The bare word `artifacts` now means stored FILES (routes/artifacts.rs,
// docs/ARTIFACTS.md); everything here carries the `identity_` prefix so the two
// never share a route, a tool name, or a store method.
//
...
pub fn router() -> Router<Store> {
    Router::new()
        ...
        // Was `/api/artifacts/{id}` — moved under /api/identity so the bare
        // path can belong to file artifacts.
        .route("/api/identity/artifacts/{id}", delete(remove))
}
...
async fn sync(State(s): State<Store>, Extension(ctx): Extension<AuthCtx>) -> ApiResult {
    let items = s.identity_artifacts_for_actor(ctx.actor()).await?;
...
async fn list(
    ...
    Ok(Json(s.identity_artifacts_list(&actor).await?).into_response())
}
...
async fn remove(
    ...
    let Some(artifact) = s.identity_artifacts_get(&id).await? else {
        return Ok(not_found());
    };
    if !can_act_for_identity(&s, &ctx, &artifact.actor).await? {
        return Ok(forbidden());
    }
    if s.identity_artifacts_remove(&id).await? {

...
async fn upsert_is_idempotent_by_actor_kind_name() {
    ...
    assert_eq!(s.identity_artifacts_list("pia").await.unwrap().len(), 1);
}
...
async fn for_actor_excludes_disabled_and_other_actors() {
    ...
    s.identity_artifacts_upsert("pia", "skill", "on", "x", "", true)
        .await
        .unwrap();
    s.identity_artifacts_upsert("pia", "agent", "off", "x", "", false)
        .await
        .unwrap();
    s.identity_artifacts_upsert("apis", "skill", "other", "x", "", true)
        .await
        .unwrap();

    let synced = s.identity_artifacts_for_actor("pia").await.unwrap();
    ...
    assert_eq!(s.identity_artifacts_list("pia").await.unwrap().len(), 2);
}
...
async fn sync_returns_only_token_actors_enabled_artifacts() {
    ...
    s.identity_artifacts_upsert("pia", "skill", "journal", "body", "j", true)
        .await
        .unwrap();
    s.identity_artifacts_upsert("pia", "agent", "scout", "body", "s", false)
        .await
        .unwrap();
    s.identity_artifacts_upsert("apis", "skill", "elsewhere", "body", "", true)
        .await
        .unwrap();
...
async fn owner_can_manage_but_non_owner_non_admin_gets_403() {
    ...
    let (status, _) = send(
        &app,
        bearer_delete(&format!("/api/identity/artifacts/{artifact_id}"), &bob_tok),
    )
    .await;
    ...
    let (status, _) = send(
        &app,
        bearer_delete(&format!("/api/identity/artifacts/{artifact_id}"), &pia_tok),
    )
    .await;

// Node boot smoke (PLAN-v2.1 PR 4.6; scenario 1 of docs/TESTING-STRATEGY.md
// §4.2): the REAL binary, a real socket, a real signal.
//
// This is the crate that owns the binary, so it is the one place
// `CARGO_BIN_EXE_hive-node` exists — multi-binary scenarios live in smoke/
// and read `HIVE_NODE_BIN` instead (§4.1). Smoke tier, so every test opens
// with `require_smoke!()` and `cargo test --workspace` stays green offline.
//
...
//   * the socket is live — a connection is accepted (and, in this PR, closed:
//     the transport lands at 4.7).
...
use hive_smoke_support::require_smoke;
...
async fn a_node_boots_announces_its_port_serves_and_exits_on_sigterm() {
    ...
    // Health: the socket is live. This PR accepts and closes (the mTLS
    // carrier is 4.7), so a read returning EOF is the expected shape.
    let mut probe = tokio::net::TcpStream::connect(node.addr)
        .await
        .expect("the listener accepts");
    let mut buf = [0u8; 1];
    let read = tokio::time::timeout(BOOT_DEADLINE, probe.read(&mut buf))
        .await
        .expect("the node answered the probe")
        .expect("reading the probe");
    assert_eq!(read, 0, "accepted and closed, per this PR's listener");
